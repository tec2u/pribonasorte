    ...
    <div class="card-body table-responsive">
      <div class="row">
        <div class="col-sm-12 col-md-6">
          <form action="{{ route('admin.users.searchUsersCorporate') }}" method="GET">
            @csrf
        ...
        <div class="col-sm-12 col-md-6">
          <div class="text-right">
            @if (isset($all_states) and !empty($all_states))
              <form action="{{ route('admin.users.filterStatesCorporate') }}" method="GET">
                @csrf
                <select name="states"
            ...
            <div class="btn-group" style="float: right;">
              ...
              <div class="dropdown-menu">
                <a class="dropdown-item" href="{{ route('admin.users.corporates') }}">All</a>
                <a class="dropdown-item"
                  href="{{ route('admin.users.filterCorporate', ['parameter' => 'activated']) }}">@lang('admin.members.filterUser.active')Activated</a>
                <a class="dropdown-item"
                  href="{{ route('admin.users.filterCorporate', ['parameter' => 'desactivated']) }}">@lang('admin.members.filterUser.desactive')Desactivated</a>
              </div>
            </div>
          </div>
        </div>
          ...
          @if (isset($filter_users) and !empty($filter_users))
            ...
            @foreach ($filter_users as $user)
              <tr>
                ...
                <td>
                  @if ($user->pay_vat == 1)
                    <button class="btn btn-success btn-sm m-0">Yes</button>
                  @else
                    <button class="btn btn-danger btn-sm m-0">No</button>
                  @endif
                </td>
                <td>{{ $user->sponsor }}</td>
                <td>{{ $user->id_corporate }}</td>
                <td>{{ $user->corporate_nome }}</td>
                <td>{{ $user->cell }}</td>
                ...
                <td>
                  <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li><a class="dropdown-item" href="{{ route('admin.users.active', ['id' => $user->id]) }}"><i
                            class="fa fa-power-off"></i>&ensp; Mark as active</a></li>
                    </ul>
                  </div>
                </td>
              </tr>
          ...
          @if (isset($users) and !empty($users))
            ...
            @foreach ($users as $user)
              @if (isset($parameter) and !empty($parameter))
                {{-- ACTIVATED --}}
                @if ($parameter == 'activated' and $user->activated == 1)
                  <tr>
                    <th>{{ $user->id }}</th>
                    {{-- <td style="text-align: center; vertical-align: middle; display: none;">
                      @if (!empty($user->image_path))
