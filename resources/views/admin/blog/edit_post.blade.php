    ...
    <div class="row d-flex justify-content-center ">
        <div class="col-lg-6">
            <div class="card card-primary">
                ...
                <form action="{{ route('admin.blog.PostsUpdate', ['id' => $post_edit->id]) }}" method="GET" enctype="multipart/form-data">
                    <div class="card-body">
                        @csrf
                        <div class="form-group">
                            <label for="name">Title</label>
                            <input type="name" class="form-control form-control-lg @error('name') is-invalid @enderror"
                                id="name" name="title" placeholder="Enter Title" value="{{ $post_edit->title }}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="category" class="form-control form-control-lg @error('category') is-invalid @enderror"
                                id="category" name="category" placeholder="Enter Category" value="{{ $post_edit->category }}">
                            @error('category')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="card-footer text-right">
                        <button type="submit" class="btn brn-lager btn-success">Editar Post</button>
                    </div>
                </form>
            </div>
        </div>
