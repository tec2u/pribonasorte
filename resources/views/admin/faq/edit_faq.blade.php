    ...
    <div class="row d-flex justify-content-center ">
        <div class="col-lg-6">
            <div class="card card-primary">
                ...
                <form action="{{ route('admin.faq.FaqUpdate', ['id' => $question_edit->id]) }}" method="GET" enctype="multipart/form-data">
                    <div class="card-body">
                        @csrf
                        <div class="form-group">
                            <label for="name">Question</label>
                            <input type="name" class="form-control form-control-lg @error('name') is-invalid @enderror"
                                id="name" name="question" placeholder="Enter Question" value="{{ $question_edit->question }}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="category" class="form-control form-control-lg @error('category') is-invalid @enderror"
                                id="category" name="category" placeholder="Enter Category" value="{{ $question_edit->category }}">
                            @error('category')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col md 12">
                            <div class="form-group">
                                ...
                                <textarea id="long_description" name="response">
                                    {{ $question_edit->response }}
                                </textarea>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer text-right">
                        <button type="submit" class="btn brn-lager btn-success">Editar question</button>
                    </div>
                </form>
            </div>
        </div>
